...
public class LeftLoadingView extends View {
    ...
    public LeftLoadingView(Context context) {
        super(context);
    }

    public LeftLoadingView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    public LeftLoadingView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 绘制进度条和叶子
        // 之所以把叶子放在进度条中绘制是因为层级关系, 进度条要盖住叶子
    ...
    private void drawLeafs(Canvas canvas) {
        ...
        for (int i = 0; i < mLeafInfos.size(); i++) {
            ...
            if (currenTime > leaf.startTime && leaf.startTime != 0) {
                // 绘制叶子, 根据叶子的类型和当前时间得出叶子的(x, y)
                getLeafLocation(leaf, currenTime);
                // 根据时间计算叶子旋转角度
                canvas.save();
                // 通过Matrix控制叶子旋转
                ...
                float rotateFraction = ((currenTime - leaf.startTime) % mLeafRotateTime) / (float)mLeafRotateTime;
    ...
    private void getLeafLocation(Leaf leaf, long currenTime) {
        long intervalTime = currenTime - leaf.startTime;
