...
object CommonUtil {
     ...
     * 获取状态栏高度
     */
    fun getStatusBarGeight(context: Context): Int {
        // 获得状态栏高度
    ...
    fun randomColor(): Int {
        val random = Random()
        // 0-190, 如果颜色值过大, 就越接近白色, 就看不起了, 所以需要限定范围
    ...
    /**
     * 解决inputMethodManager 引起的内存泄露
     * 在Activity 的 onDestory 方法里调用
     */
            ...
            try {
                ...
                if (objGet != null && objGet is View) {
                    ...
                    if (view.context === context) {
                        // 被InputMethodManager持有引用的 context 是想要目标销毁的
