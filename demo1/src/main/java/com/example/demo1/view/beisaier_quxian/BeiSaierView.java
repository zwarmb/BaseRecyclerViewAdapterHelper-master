...
//    private Paint mContralPaint = null;     // 控制点画笔
...
//    private static final int RATE = 10;             //移动速度
...
//                    mR = 0;
//                    mState &= ~STATE_RUNNING;       // 这个写法和 +=, -= 类似, 只是用的较少, 是布尔运算符
//                    mState &= ~STATE_STOP;
...
//    private void init() {
//        // 初始坐标
//        mControlPoints = new ArrayList<>(MAX_COUNT);
...
//
//        // 控制点画笔
//        mContralPaint = new Paint();
//        mContralPaint.setColor(Color.BLACK);
//        mContralPaint.setAntiAlias(true);
//        mContralPaint.setStyle(Paint.Style.STROKE);
//
//        // 切线画笔
...
//        for (int i = 0; i < 1; i++) {
//            // Bezier点集
//            points.add(new PointF(deCasteljauX(order, 0, i), deCasteljauY(order, 0, i)));
//        }
//        return points;
//    }
//
...
//     * @return
//     */
//    private float deCasteljauX(int order, int i, float i1) {
//        if (order == 1) {
//            return (1 - i1)*mControlPoints.get(i).x + i1*mControlPoints.get(i + 1).x;
//        }
//        return (1 - i1)*deCasteljauX(order - 1, i, i1) + i1 * deCasteljauX(order - 1, i + 1, i1);
//    }
//
//    /**
//     * deCasteljau 算法
...
//     * @return
//     */
//    private float deCasteljauY(int order, int i, float i1) {
//        if (order == 1) {
//            return (1 - i1)*mControlPoints.get(i).y + i1*mControlPoints.get(i + 1).y;
//        }
//        return (1 - i1)*deCasteljauY(order - 1, i, i1) + i1*deCasteljauY(order - 1, i + 1, i1);
//    }
//
//
...
//                    }
//                    float x = (1 - k) * p0x + k * p1x;
//                    float y = (1 - k) * p0y + k * p1y;
...
//        for (PointF pointF: mControlPoints) {
//            if (mCurPoint != null && mCurPoint.equals(pointF)) {    // 判断是否是当前控制点
//                continue;
//            }
//            rectF.set(pointF.x - REGION_WIDTH, pointF.y - REGION_WIDTH, pointF.x + REGION_WIDTH, pointF.y + REGION_WIDTH);
...
//        if (mCurPoint != null) {
//            RectF rectF = new RectF(mCurPoint.x - FINGER_RECT_SIZE / 2, mCurPoint.y - FINGER_RECT_SIZE / 2,
//                    mCurPoint.x + FINGER_RECT_SIZE / 2, mCurPoint.y + FINGER_RECT_SIZE / 2);
//            if (rectF.contains(x, y)) {
//                return true;
//            }
//        }
...
//                    if (isLegalFingerRegion(x, y)) {                    // 判断手指出没区域是否合法
//                        mCurPoint.x = x;
//                        mCurPoint.y = y;
//                        invalidate();
//                    }
...
//            int r = mWidth / 5;
//            float[][] region = {{0, r}, {0, -r}, {r, r}, {-r, -r}, {r, 0}, {-r, 0}, {0, 1.5f * r},
//                    {0, -1.5f * r}, {1.5f * r, 1.5f * r}, {-1.5f * r, -1.5f * r}, {1.5f * r, 0},
//                    {-1.5f * r, 0}, {0, 2 * r}, {0, -2 * r}, {2 * r, 2 * r}, {-2 * r, -2 * r}, {2 * r, 0}, {-2 * r, 0}};
//            int t = 0;
